// ==UserScript==
// @name         EZ Logi Con Count v1.3.1
// @namespace    http://tampermonkey.net/
// @version      1.3.1
// @description  Inventory management for DCO consumables — CMH73, synced to SharePoint Excel
// @match        *://*/*
...
(function () {
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        const token = prompt(
          ...
          'Paste a Graph API token with Files.ReadWrite scope\n' +
          '(or cancel to use offline/local mode):'
        );
        if (token) { accessToken = token; resolve(); }
        else reject(new Error('No token — offline mode'));
      } catch (e) {
        reject(e);
      }
    });
  ...
  function showQRModal(localIdx) {
    ...
    const min      = Number(row[COL.MIN]);
    const max      = Number(row[COL.MAX]);
    const qrData   = JSON.stringify({ building, item, min, max });
