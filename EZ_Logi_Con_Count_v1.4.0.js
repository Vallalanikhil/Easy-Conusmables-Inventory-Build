// ==UserScript==
// @name         EZ Logi Con Count v1.4.0
// @namespace    http://tampermonkey.net/
// @version      1.4.0
// @description  Inventory management for DCO consumables — CMH73 | Boost only
...
(function () {
  ...
  function launchApp() {
    getAccessToken()
      ...
      .then(() => fetchFromSharePoint())
      .then(() => {
        spAvailable = true;
        saveLocalCache();
        showToast('SharePoint synced ✓', 'success');
      })
      .catch(err => {
        console.warn('[EZ Logi] SharePoint unavailable:', err.message);
        loadLocalFallback();
  ...
  // ─── AUTH ─────────────────────────────────────────────────────────────────
  // Attempts to grab MSAL token silently from session storage.
  // If unavailable, falls through to offline/local cache mode — no prompt shown.
  function getAccessToken() {
    return new Promise((resolve, reject) => {
      try {
        ...
        // No token found — silently fall to offline mode
        reject(new Error('No token — offline mode'));
      } catch (e) {
        reject(e);
      }
    });
  ...
  // ─── AUTO-DISCOVER SITE + DRIVE ───────────────────────────────────────────
  function discoverSiteAndDrive() {
    if (CONFIG.SITE_ID && CONFIG.DRIVE_ID) return Promise.resolve();
    return graphRequest('GET', `https://graph.microsoft.com/v1.0/sites/${CONFIG.TENANT_HOST}:${CONFIG.OWNER_PATH}`)
      .then(siteData => {
        CONFIG.SITE_ID = siteData.id;
        GM_setValue('site_id', siteData.id);
        return graphRequest('GET', `https://graph.microsoft.com/v1.0/sites/${CONFIG.SITE_ID}/drives`);
      })
      .then(drivesData => {
  ...
  // ─── PUSH TO SHAREPOINT ───────────────────────────────────────────────────
  function pushToSharePoint() {
    if (!spAvailable || !accessToken) {
      showToast('SharePoint not available — saved locally only', 'info');
      return Promise.resolve();
    }
    const values   = [headerRow, ...allRows];
    const rowCount = values.length;
    const colCount = headerRow.length || COL.MIN + 1;
    ...
    return graphRequest('PATCH', rangeUrl, { values })
      .then(() => showToast('SharePoint synced ✓', 'success'))
      .catch(err => showToast('SharePoint sync failed: ' + err.message, 'error'));
  }
  ...
  function loadLocalFallback() {
    ...
    if (savedRows) {
      ...
      headerRow = savedHeader ? JSON.parse(savedHeader) : ['Building','Item Name','Current Qty','Max Qty','Min Threshold'];
    } else {
      allRows   = SEED_DATA.map(r => [...r]);
      headerRow = ['Building','Item Name','Current Qty','Max Qty','Min Threshold'];
      saveLocalCache();
    }
    inventory = allRows.filter(r => r[COL.BUILDING] === userBuilding);
  }
  ...
  function renderPanel() {
    ...
    panel.innerHTML = `
      <div class="inv-header">
        ...
        <span class="inv-sp-badge" id="inv-sp-badge" title="${spAvailable ? 'SharePoint connected' : 'Offline — local cache'}">${spAvailable ? '🟢' : '🟡'}</span>
        <button class="inv-btn-icon" id="inv-sync-now" title="Sync to SharePoint now">🔄</button>
    ...
    makeDraggable(panel);
    document.getElementById('inv-sync-now').addEventListener('click', () => { saveLocalCache(); pushToSharePoint(); });
  ...
  function triggerSlackAlert(row) {
    ...
    const payload = { building, item, current, reorder, max };
